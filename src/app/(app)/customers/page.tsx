        ...
        {customers.map((customer) => {
          ...
          return (
            <Link key={customer.id} href={`/customers/${customer.id}`}>
              <Card className="p-6 transition hover:-translate-y-0.5 hover:border-[#22ddeb] hover:shadow-[0_12px_28px_rgba(15,23,42,0.08)]">
                <div className="flex items-center gap-4">
                  <Avatar name={customer.name} src={customer.avatarUrl} />
                  <div><div className="font-bold text-[#f8fbff]">{customer.name}</div><div className="text-sm text-[#6f858f]">{customer.phone}</div></div>
                </div>
