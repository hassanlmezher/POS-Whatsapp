  ...
  if (message.messageType === "image" && attachment?.url) {
    return (
      <div className="min-w-[240px] max-w-[420px]">
        ...
        <div className={`mt-2 flex items-center justify-between gap-3 text-xs ${message.direction === "outbound" ? "text-black/65" : "text-[#000000]"}`}>
    ...
    <div className="min-w-[260px] max-w-[420px]">
      <div className={`flex items-center gap-4 rounded-lg border p-4 ${
        message.direction === "outbound"
          ? "border-[#d8c3ff] bg-[#ffffff]"
          : "border-[#d8c3ff] bg-[#ffffff]"
      }`}>
        <div className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-lg ${
          message.direction === "outbound" ? "bg-[#f4ecff] text-[#7c3aed]" : "bg-[#f4ecff] text-[#7c3aed]"
        }`}>
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className={`mt-1 text-xs ${message.direction === "outbound" ? "text-black/60" : "text-[#000000]"}`}>
...
function AttachmentStatus({ label }: { label: string }) {
  return (
    <div className="mt-3">
      ...
      <div className="h-1.5 overflow-hidden rounded-full bg-[#f4ecff]">
        <span className="block h-full w-1/2 animate-[progress-slide_1s_ease-in-out_infinite] rounded-full bg-[#7c3aed] opacity-70" />
      </div>
    </div>
  );
}
...
function AttachmentError({ error }: { error: string | null | undefined }) {
  return (
    <div className="mt-3 rounded-lg border border-[#7c3aed] bg-[#f4ecff] px-3 py-2 text-xs font-semibold text-[#6d28d9]">
