...
export function Header({
  ...
  return (
    <header className="sticky top-0 z-20 flex h-[98px] items-center justify-between gap-4 border-b border-[#d8c3ff] bg-[#ffffff]/95 px-4 backdrop-blur lg:px-8">
      <div className="min-w-0">
        <p className="text-xs font-black uppercase tracking-[0.18em] text-[#7c3aed]">
          InChouf POS
        </p>
        <h1 className="mt-1 truncate text-xl font-black text-[#000000] sm:text-2xl">
          {context.title}
        </h1>
        <p className="mt-1 hidden max-w-[560px] truncate text-sm text-[#000000] md:block">
          {context.description}
        </p>
      </div>

      <div className="ml-auto flex items-center justify-end gap-3 text-[#000000] sm:gap-5">
        <button className="rounded-lg p-2 transition hover:bg-[#f4ecff] hover:text-[#7c3aed]" aria-label="Notifications">
          <Bell className="h-6 w-6" />
        </button>
        <button className="hidden rounded-lg p-2 transition hover:bg-[#f4ecff] hover:text-[#7c3aed] sm:inline-flex" aria-label="Help">
          <CircleHelp className="h-6 w-6" />
        </button>
        <button className="hidden rounded-lg p-2 transition hover:bg-[#f4ecff] hover:text-[#7c3aed] sm:inline-flex" aria-label="Applications">
          <Grid3X3 className="h-6 w-6" />
        </button>
        <div className="hidden h-14 w-px bg-[#d8c3ff] md:block" />
        <Link
          href="/pos"
          className="inline-flex h-12 items-center justify-center gap-2 rounded-lg bg-[#7c3aed] px-4 text-sm font-semibold text-black shadow-[0_6px_14px_rgba(124,58,237,0.24)] transition hover:bg-[#6d28d9] sm:px-6 sm:text-base"
        >
